...
namespace Clustering
{
    ...
    public class PointCluster<labelType>
    {
        ...
        unsafe public static float DistanceBetween(int c, float[] arr, int numClusters, out int bestpairIdx)
        {
            float cX;
            float cY;
            float bestdist = float.PositiveInfinity, dist;
            bestpairIdx = -1;
            ...
            fixed (float* arrFixed = arr)
            {
                ...
                {
                    deltaX2 = cX - arrFixed[i * 2 + 0];
                    deltaY2 = cY - arrFixed[i * 2 + 1];
                    deltaX2 *= deltaX2;
                    deltaY2 *= deltaY2;
                    //float dist = (float)Math.Sqrt(deltaX2 + deltaY2);

                    //dist = FastSqrt.Sqrt(deltaX2 + deltaY2);
                    dist = deltaX2 + deltaY2;
                    if (dist < bestdist)
                    {
                        if (c != i)
                        {
                            bestpairIdx = i;
                            bestdist = dist;
                            bestdist = FastSqrt.Sqrt(deltaX2 + deltaY2);
                        }
                    }
                }
            }
