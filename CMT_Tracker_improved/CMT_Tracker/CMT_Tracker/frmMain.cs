...
namespace CMT_Tracker
{
    public partial class frmTrackMain : Form
    {
       ...
       // private static Brisk featureDetector = new Brisk(30, 3, 1.0f);
        ...
        private StreamWriter outputFile, fps;
        ...
        public frmTrackMain()
        {
            ...
            {
                InitializeComponent();
                File.Create(@"Debug\totals.txt");
                File.Create(@"Debug\process.txt");
                File.Create(@"Debug\keypoints.txt");
                fps = new StreamWriter("FPS.txt");
            }

            FileInfo assembly = new FileInfo(System.Reflection.Assembly.GetExecutingAssembly().Location);

            //We want to use CMT SURF
           /* mTracker = new CMT();
            ...
            mTracker.OpticalFlowFunction = new CMT.ComputeOpticalFlowDelegate(ExternalComputeLKOpticalFlow);*/
             ...
             mTracker = new CMT_SURF();
            // Connect external delegate functions
             //mTracker.ExtractKeypoints = new CMT_SURF.ExtractKeypointDelegate(ExternalBRISKKeypoints);
             //mTracker.ExtractKeypointDescriptors = new CMT_SURF.ExtractKeypointDescriptorDelegate(ExternalBRISKDescriptors);

           //  mTracker.OpticalFlowFunction = new CMT_SURF.ComputeOpticalFlowDelegate(ExternalComputeLKOpticalFlow);

            mTracker.EstimateScale = true;
            mTracker.EstimateRotation = true;
        ...
        private int CaptureNextFrame()
        {
            ...
            {
                nextFrame = String.Format("{0}{1}.png", sourcePath, mNextFrame++.ToString("000000000"));
            }
            if (File.Exists(nextFrame))
            {
                ...
                if (!mROIMode)
                {
                    ...
                    sw.Stop();
                    //using (StreamWriter stream = File.AppendText(@"Debug\totals.txt"))
                   // {
                     //   stream.WriteLine("{0}", sw.ElapsedMilliseconds);
                  //  }
                    //Console.WriteLine("Frame processed in: {0}", sw.Elapsed);
                    TimeSpan g = sw.Elapsed;
                    double ti = g.TotalSeconds;
                    fps.WriteLine("{0}", 1/ti);
