...
namespace OpenSURF
{
    public class OpenSURFDescriptor
    {
      ...
      {0.00131955648461f,0.00103799989504f,0.00069579213743f,0.00039744277546f,0.00019345616757f,0.00008024231247f,0.00002836202103f}
        ...
        public static void DecribeInterestPoints(List<OpenSURFIPoint> ipts, bool upright, bool extended, OpenSURFIntegralImage img)
        {
            OpenSURFDescriptor des = new OpenSURFDescriptor();
        ...
        /// <param name="ipts"></param>
        /// <param name="upright"></param>
        public void DescribeInterestPoints(List<OpenSURFIPoint> ipts, bool upright, bool extended, OpenSURFIntegralImage img)
        {
            if (ipts.Count == 0) return;
            this.img = img;

            foreach (OpenSURFIPoint ip in ipts)
            {
        ...
        /// </summary>
        /// <param name="ip"></param>
        unsafe void GetOrientation(OpenSURFIPoint ip)
        {
                ...
                for (int i = -6; i <= 6; ++i)
                {
                    for (int j = -6; j <= 6; ++j)
                    {
                        if (i * i + j * j < 36)
                        {
                            ...
                            resX[idx] = gauss * img.HaarX(ptrMatrix, Y + j * S, X + i * S, 4 * S);
                            resY[idx] = gauss * img.HaarY(ptrMatrix, Y + j * S, X + i * S, 4 * S);
        ...
        unsafe void GetDescriptor(OpenSURFIPoint ip, bool bUpright, bool bExtended)
        {
                    ...
                    fixed (float* ptrMatrix = IIMatrix)
                    {
                        for (int k = i; k < i + 9; ++k)
                        {
                            for (int l = j; l < j + 9; ++l)
                            {
                                ...
                                rx = (float)img.HaarX(ptrMatrix, sample_y, sample_x, 2 * S);
                                ry = (float)img.HaarY(ptrMatrix, sample_y, sample_x, 2 * S);
